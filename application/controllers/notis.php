 ...
 *	(7 Jan 2016)	:	1. Kemaskini method kemaskini_rekod() supaya rekod boleh dikemaskini untuk sebarang status aktif/tidak aktif
 */
...
class Notis extends MY_Controller {
    ...
    function selenggara_notis(){
       ...
       // $data['papar'] = $this->Eminda_model->get_info('notisPeringatan', array('idPengguna'=>$id, 'lokalitiAdmin'=>$lokaliti));
		$data['papar'] = $this->Eminda_model->get_info('notisPeringatan', array( 'lokalitiAdmin'=>$lokaliti));
        ...
        if($this->form_validation->run() == true){	
			...
			$countnokp = $this->Eminda_model->get_count('notisPeringatan',array('lokalitiAdmin'=>$lokaliti));
            if($countnokp != 0){
    ...
    function selenggara_emel(){		 
        ...
        $data['id'] =  $id;
        $data['lokaliti'] =  $lokaliti;
        $data['perihal_lokaliti'] =  $this->Tbl_pengguna_model->get_perihal_lokaliti($id);
        // untuk post data
		...
		if($this->form_validation->run() == true){			  			  
            $countnokp = $this->Eminda_model->get_count('penerimaNotis',array('lokalitiAdmin'=>$lokaliti,'statusPN'=>1));
			//echo $countnokp;
			//exit;
			//echo "testing";
            if(($this->input->post('status')=='0')||($countnokp < 2)) {
                ...
                flashMsg('Rekod Berjaya Disimpan.','success');
                redirect('notis/selenggara_emel/'.$id);
            } else {			   
    ...
    function kemaskini_rekod($id){		 
		...
		if(($this->input->post('status')=='1')&&($countnokp <2))
		{
		...
		else if(($data['kemaskini']['statusPN']==$this->input->post('status'))||($this->input->post('status')=='0'))
		{
		$this->Tbl_penerimanotis_model->update_data('penerimaNotis',array('ID'=>$id), $data['update']);
		flashMsg('Rekod Berjaya Disimpan.','success');
		redirect('notis/selenggara_emel');
