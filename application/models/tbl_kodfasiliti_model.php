 ...
 *  (3 Nov 2015)    :   1. Betulkan masalah pada menu laporan sekiranya pengguna
 *                         daftar masuk sebagai admin
 */
...
class Tbl_kodfasiliti_model extends CI_Model {
	...
	function get_fasiliti_perihal($lokaliti){
        $this->db->select('perihalFasiliti');
        $this->db->from($this->tableName);
        $this->db->where('kodFasiliti',$lokaliti);        
        return $this->db->get()->row('perihalFasiliti');        
    }//end method...
    ...
    public function get_fasiliti($lokaliti){
        ...
        if($num==0){
            $this->db->select('kodJenisFasiliti');
            $this->db->from($this->tableName);
            $this->db->where('kodJenisFasiliti',$lokaliti);
            $this->db->order_by($this->fk,'ASC');
            $this->db->group_by($this->fk);
            $query2 = $this->db->get();
	...
	function get_jenisfasiliti_kod($lokaliti){
        $this->db->select('kodJenisFasiliti');
        $this->db->from($this->tableName);
        $this->db->where('kodFasiliti',$lokaliti);        
        return $this->db->get()->row('kodJenisFasiliti');        
    }//end method...
