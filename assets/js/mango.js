...
(function($$, $, undefined){
	...
	var UM = {
		...
		utils: {
			...
			loaded: function(cb) {
				...
				$.isFunction(cb) ? _cb.push(UM.utils.tryF(cb)) : _cb.forEach(function(f){f()});
		...
		// ! No Conflict Mode:
		// - Return $$ to it's previous owner and return the UM object
		noConflict: function(){
			window[$$] = old$$;
			return this;
		}
	...
	// - Publish functions
	UM.loaded = UM.utils.loaded;
	UM.ready = UM.utils.ready;
