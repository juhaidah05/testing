...
(function($, window, document){
	...
	$.fn[PLUGIN_NAME].defaults = {
		...
		tableTools: {
			...
			swfUrl: base_url+'public/extras/datatables/copy_csv_xls_pdf.swf',	// Where the swf file is located
...
//			pos: 'right', 							// Show on the left or on the right?
//			swfUrl: 'public/extras/datatables/copy_csv_xls_pdf.swf',	// Where the swf file is located
//			extras: {}								// Other tableTools options
//		},
		lang: {
		...
		return $(this).each(function(){
			...
			$table.dataTable($.extend(true, {
				...
				sPaginationType: 'full_numbers',
                                bProcessing:false,
                                bDestroy:true,
                                 bServerSide: true,bRetrieve: true,bInfo: true,
                                sAjaxSource:opts.url,
                                sServerMethod: 'GET',
				iDisplayLength: opts.maxItemsPerPage,
                                fnServerParams: function ( aoData ) {
                                if(!empty(opts.Userdata)){
                                  opts.Userdata= JSON.stringify(opts.Userdata);
                                opts.Userdata=JSON.parse(opts.Userdata);                             
                                $(opts.Userdata).each(function(key,val){
                                    $.each(val,function(keyv,valv){
                                  aoData.push({ "name": keyv, "value": valv});
                                  });
                                 });  
                                }
...
(function($, UM, window, undefined){

	UM.settings = function(){
			UM.settings.el.dialog({
				modal: true,
				draggable: false,
				width: UM.settings.el.data('width') || UM.config.settings.width,
...
(function($, UM, _, window, document, undefined) {
		...
		$.extend(self, {
			...
			init: function(opts){
				...
				self.sliderTimer = function(){
					...
					}, UM.config.lock.idle * 1000); // * 1000 -> sec to ms
				...
				UM.config.lock.lockWhenInactive && document.addEventListener(self.pageVisibility.visibilityChange, function(){
			...
			start: function(){
				...
				// Set up shorthands
				var timeout = UM.config.lock.timeout; // Shorthand
			...
			utils: {
				...
				formatSeconds: function(seconds) {
                                    ...
                                    var Min=currentdate.getMinutes();
                                        ...
                                        if(Hr.length==1){
                                           Hr="0"+currentdate.getMinutes(); 
                                        }
                                         if(Min.length==1){
                                           Min="0"+currentdate.getHours(); 
                                        }
                                        return  Hr + ":" +Min+":"+Sec ;
...
(function($, _, window, document, undefined) {
	...
	$.fn[PLUGIN_NAME] = $.extend(function (method, options) {
		...
		defaults: {
			...
			type: 'bar',	// One of 'bars', 'lines', 'pie'
			...
			tooltip: function(label, xlabel, xval, yval){
				return (label ? (label + " View On  ") : '') + (this.options.flot.xaxis.ticks || (this.options.horizontal && this.options.flot.yaxis.ticks) ? (xlabel != '' ? xlabel : xval.toFixed(2)) + " = " : '') + yval+" end Users Details ";
